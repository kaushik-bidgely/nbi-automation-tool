...
const ActionRow = memo(function ActionRow({
  ...
  const overLimit = Object.entries(dirty).some(
    ([field, value]) => ACTION_CHAR_LIMITS[field] && value.length > ACTION_CHAR_LIMITS[field],
  );
...
export default function ActionsList() {
  ...
  const toggleSelectAll = () => {
    setSelected((s) => (filtered.every((a) => s.has(a.id)) ? new Set() : new Set(filtered.map((a) => a.id))));
  };
  ...
  const downloadSheet = async (statuses: string[]) => {
    if (!pilotId) return;
    const res = await api.exportActionsSheet(pilotId, statuses);
    downloadCsv(res.filename, res.csv);
  };
